...
double static_eval(Node &root)
{
    ...
    int i, j;
// #pragma omp parallel for collapse(2) private(i, j) schedule(dynamic) reduction (+:score)
    for (i = 2; i < 10; i++)
    {
        for (j = 2; j < 10; j++)
        {
            char c = root->board[i][j];
            // cout<<"current thread in evaluate func outer loop = "<<  omp_get_thread_num()<<endl;
            // cout<<" inner loop = "<<  omp_get_thread_num()<<endl;
